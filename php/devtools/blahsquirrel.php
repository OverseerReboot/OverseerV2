...
if ($accrow['modlevel'] < 99) {
    ...
    if (!empty($_POST['query'])) {
        ...
        if (strpos($_POST['query'], "SELECT") !== false) {
            ...
            if (!$result) {
                echo "We got an error... " . mysqli_error($connection);
            } else {
    ...
    $tablenames = mysqli_query($connection, "SELECT `table_name` FROM `information`_schema.tables WHERE `table_schema` = 'OverseerDev';");
