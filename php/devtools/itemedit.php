...
function anyglitch()
{
    $gtype = rand(1, 5);
    if ($gtype == 5) {
        $glitch = generateGlitchString();
    } elseif ($gtype == 4) {
        $glitch = generateStatusGlitchString();
    } else {
        $glitch = "";
        while ($gtype > 0) {
            $glitch .= horribleMess();
...
if ($accrow['modlevel'] < 4) {
    ...
    if (!empty($_POST['publishlog'])) {
        if ($accrow['modlevel'] >= 6) {
            ...
            echo "You don't have permission to post an addlog.<br />";
        }
    }
    ...
    if (!empty($_POST['code'])) {
        if (!empty($_POST['weighted'])) {
            ...
            $totalweight = 0;
            foreach ($grist as $g) {
                if (!empty($_POST[$g['name']])) {
                    $totalweight += intval($_POST[$g['name']]);
                }
            }
            if (!empty($_POST['weightround'])) {
                $round = intval($_POST['weightround']);
            } else {
                $round = 1;
            }
            foreach ($grist as $g) {
            ...
            while ($field = mysqli_fetch_field($fieldresult)) {
                ...
                if ($fname == 'code') {
                    ...
                    if (!$founditem) {
                        ...
                        if ($_POST['populate'] == "no") {
                            ...
                            if ($_POST['processing'] == 0) {
                                ...
                                echo 'Check to see if the submission has the correct code for its recipe.<br />';
                            }
                            $blocked = true;
                            break;
                        } else {
                    ...
                    if ($fname == "gristcosts") {
                        ...
                        if (empty($gstring)) {
                            $gstring = "Build_Grist:0|";
                        }
                        $_POST[$fname] = $gstring;
                    }
                    if ($fname == "nonsense") {
                        if (empty($_POST['nonsense'])) {
                            $_POST['nonsense'] = anyglitch();
                        } else {
    ...
    if (!empty($_GET['sub']) && !$victory) {
        ...
        if ($feedrow['greenlight'] == 1) {
            ...
            if (!empty($feedrow['abstratus'])) {
                ...
                if (strpos($erow['abstratus'], "kind") !== false) {
                    $weapon = true;
                }
            } else {
            ...
            if (!empty($feedrow['wearable'])) {
                ...
                if (strpos($erow['wearable'], "body") !== false) {
                    $bodygear = true;
                }
            } else {
                $erow['wearable'] = "none";
            ...
            echo "Either no submission with that ID exists, or it isn't ready to be processed.<br />";
        }
    }
    //print_r($suggrist);
    ...
    while ($field = mysqli_fetch_field($fieldresult)) {
        ...
        if ($fname == "description") {
            ...
            if ($founditem) {
                ...
                echo ' value="average"';
            } elseif ($fname == "abstratus") {
                echo ' value="notaweapon"';
            } elseif ($fname == "wearable") {
                echo ' value="none"';
            }
            echo '></td></tr>';
        }
    ...
    foreach ($grist as $g) { //go through the grists now that they're being done differently
        ...
        $gristname = $g['name'];
        if ($g['gif'] == 1) {
            $gristimg = $gristname . ".gif";
        } else {
            $gristimg = $gristname . ".png";
        }
    ...
    if ($ctgc != 0) {
        echo "Current total grist cost: $ctgc<br />";
    }
