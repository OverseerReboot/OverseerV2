...
if ($accrow['modlevel'] < 99) {
    ...
    $exploded = explode("|", urldecode($announcements['announcements']));
    if ($exploded[0] != '') {
        foreach ($exploded as $index => $ann) {
            $anno = explode("@", $ann);
            if ($anno[0] != '') {
                echo "Announcement " . $index . ":<br> " . stripcslashes(urldecode($anno[0])) . "<br>From: " . date("F j, Y, g:i a", $anno[1]) . "<br>To: " . date("F j, Y, g:i a", $anno[2]) . "<br>";
