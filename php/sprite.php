...
if (empty($_SESSION['character'])) {
    ...
    if (!empty($_POST['protoobj'])) { //player is prototyping something
        ...
        if ($power > 999 || $power < -999) {
            ...
            if (empty($_POST['protoname'])) {
                $name = "Sprite";
            } else {
                $name = $_POST['protoname'];
            }
            $obj = $_POST['protoobj'];
            if (empty($_POST['protoname'])) {
                $desc = "A prototyped sprite.";
            } else {
                $desc = $_POST['protodesc'];
            }
            if ($charrow['inmedium'] == 0) {
                if (empty($_POST['protoenemydesc'])) {
                    $enemydesc = "It has appearance aspects from $obj.";
                } else {
                    $enemydesc = $_POST['protoenemydesc'];
                }
            }
            ...
            while ($i <= 4) { //4 = max number of effects
                ...
                switch ($_POST[$thiseff]) {
                    case "TIMESTOP":
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "TIMESTOP@2:" . strval($chance) . ":Time:%USER% freezes %TARGET% in time!|";
                        }
                        break;
                    case "HOPELESS":
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "HOPELESS@0:" . strval($chance) . ":Hope:%USER%'s attack causes %TARGET% to lose hope!|";
                        }
                        break;
                    case "KNOCKDOWN":
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "KNOCKDOWN:" . strval($chance) . "|";
                        }
                        break;
                    case "WATERYGEL":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "WATERYGEL@4:" . strval($chance) . ":Life:%USER%'s attack lowers %TARGET%'s gel viscosity!|";
                        }
                        break;
                    case "POISON":
                        ...
                        if (!empty($exist[0][0])) { //since poison increases severity rather than chance, handle stacking here
                            ...
                            $effectstr .= "POISON@5@" . strval($severity) . ":10:Doom:%USER%'s attack poisons %TARGET%!|";
                        }
                        break;
                    case "BLEEDING":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "BLEEDING@5:" . strval($chance) . ":Blood:%USER%'s attack inflicts a deep wound on %TARGET%!|";
                        }
                        break;
                    case "DISORIENTED":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "DISORIENTED@4:" . strval($chance) . ":Mind:%TARGET% is disoriented by %USER%'s attack!|";
                        }
                        break;
                    case "DISTRACTED":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "DISTRACTED@2:" . strval($chance) . ":Mind:%TARGET% is distracted by %USER%!|";
                        }
                        break;
                    case "ENRAGED":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "ENRAGED@4:" . strval($chance) . ":Rage:%TARGET% is enraged by %USER%'s attack!|";
                        }
                        break;
                    case "MELLOW":
                        $chance = floor($powerchunk / 10);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "MELLOW@4:" . strval($chance) . ":Rage:%USER% causes %TARGET% to chill out, man...|";
                        }
                        break;
                    case "GLITCHED":
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "GLITCHED@0:" . strval($chance) . ":Void:%USER%'s attack causes %TARGET% to glitch out!|";
                        }
                        break;
                    case "UNLUCKY":
                        $chance = floor($powerchunk / 20);
                        ...
                        if (empty($exist[0][0])) {
                            $effectstr .= "UNLUCKY@0:" . strval($chance) . ":Light:%USER% introduces %TARGET% to a world of misfortune!|";
                        }
                        break;
            ...
            echo "Your sprite is already fully developed! It cannot accept further prototypings.<br />";
        }
