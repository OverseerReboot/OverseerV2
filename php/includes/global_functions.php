...
function getStat($char, $stat): string|null
{
    ...
    foreach ($stats as $existing_stat) {
        ...
        if ($result[0] == $stat) {
            return $result[1];
        } //if any stat found is $stat, return its value
    }
...
function updateStat($char, $stat, $value): void
{
    if (getStat($char, $stat) !== null) {
        ...
        writeStat($char, $char['stats'] . $stat . ':' . $value . '|');
    } //no point in even searching the row again
}
...
function maxStat($char, $stat, $value): void
{
    ...
    if ($top === null or $top < $value) {
        updateStat($char, $stat, $value);
    }
}
...
{
    if ($name == "Rainbow" || $name == "Polychromite" || $name == "Opal" || $name == "Plasma") {
        $name .= ".gif";
    } else {
        $name .= ".png";
    }
...
function getChar($charid)
{
    ...
    if ($charid == -1) {
        return array("name" => "[MISSING PLAYER]", "colour" => "000000");
    }
...
function strifeInit($charrow): void
{
    ...
    while (!empty($equips[$i])) { //sort through player's equipped, er, equipment
        ...
        if ($wid != 0) { //if there's actually something equipped here
            ...
            if ($weapon) { //is a weapon, add power
                ...
                if ($equip[0] == "off") {
                    $powerplus = $powerplus / 2;
                } //offhand weapons only add half the weapon's power
                $wakepower += $powerplus;
    ...
    if (strpos($wakeeffects, "COMPUTER") !== false) {
        $equippedcomputer = 1;
    }
...
{
    $computability = 0;
    if (strpos($charrow['storeditems'], "ISCOMPUTER.") !== false) {
        $computability = 1;
    }
    if ($charrow['hascomputer'] != 0) {
        $computability = 2;
    }
    if ($charrow['equippedcomputer'] == 1) {
        $computability = 3;
    }
