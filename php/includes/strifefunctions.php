...
function generateEnemies($enemylist, $strifeID, $connection, $appearson, $generateleader, $sessionID = 0, $landID = 0, $tierlist = "", $persist = 0): bool
{
    ...
    while ($row = mysqli_fetch_array($enemyresult)) {
        if (strpos($appearson, $row['appearson']) === false && $appearson != "ANY") {
            return false;
        } //Enemy not permitted
    ...
    while (!empty($enemylist[$i])) {
        ...
        } //The first enemy in the list is the leader IF generateleader is 1.
        ...
        if ($enemyrow['underling'] == 1) { //This enemy is an underling. Set up their drops, power, and health accordingly.
            ...
            if ($enemyrow['maxboons'] > 0) {
                $ondeath .= "BOONDOLLARS:$enemyrow[minboons]:$enemyrow[maxboons]|";
            } //Set Boondollar drop if it exists
        }
...
function powerCalc($strifer): array
{
    ...
    if ($strifer['fatigue'] > 1025) {
        if ($strifer['fatigue'] > 1500) {
            setAchievement(getChar($strifer['owner']), 'fatigue');
        }
...
{$strifer['strifeID'] = 0; //We win! No more strife
    ...
    while (!empty($motifsused[$j])) { //Decrement the cooldown. If it's now zero, remove the fraymotif's entry.
        ...
        if ($currentmotif[1] > 0) {
            $newstr .= "|" . $currentmotif[0] . ":" . $currentmotif[1];
        }
        $j++;
    }
