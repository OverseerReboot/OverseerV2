...
if (empty($_SESSION['username'])) {
    ...
    if (empty($_SESSION['motiflist']['Aspect'])) {
    ...
    if (!empty($_POST['motifbuy'])) { //Fraymotif being purchased. DATA SECTION: Stores costs for fraymotifs
        ...
        if ($_SESSION['motiflist'][$_POST['motifbuy']] != "") {
            $motifname = $_SESSION['motiflist'][$_POST['motifbuy']];
        }
        ...
        if ($cost <= $charrow['boondollars']) {
            if ($charrow['fraymotifs'] == "") {
                $charrow['fraymotifs'] = "|";
            } //Add a leading | to allow for strpos lookup of fraymotifs later on.
    ...
    while (!empty($motifs[$i])) { //Owned fraymotif. Print a designation and provide an option to use it.
        ...
        if ($motifowned[$motifs[1]] and $motifowned[$motifs[2]] and $motifowned[$motifs[3]]) {
            setAchievement($charrow, 'fray3');
        }
