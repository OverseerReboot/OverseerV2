...
foreach ($characterids as $characterid) {
    ...
    echo('<tr>');
    if ($curchar['server'] == $curchar['client']) {
...
foreach ($characterids as $i) {
    if ($charused[$i] == false) {
        $charused[$i] == true;
        if ((getchar($i)['server'] == 0) && (getchar($i)['client'] == 0)) {
            $nochain .= profileStringSoft($i).' is not part of a chain.'."<br>\n";
        } else {
            ...
            if (getChar($i)['client'] != 0) {
                ...
                while ($charused[$curchar] == false) { // check to make sure character hasn't already been iterated over
                    ...
                    if (getChar($curchar)['client'] != 0) {
                        $curchar = getChar($curchar)['client'];
                    } else {
                        $curchar = $i;
                    }
                }
            } else {
                echo('Player has no client.'."\n");
            }
            if (getChar($i)['server'] != 0) {
                ...
                while ($charused[$curchar] == false) { // check to make sure character hasn't already been iterated over
                    $chainlist = '  '.profileStringSoft($curchar)."<br>\n".$chainlist;
                    $charused[$curchar] = true;
                    if (getChar($curchar)['server'] != 0) {
                        $curchar = getChar($curchar)['server'];
                    } else {
                        $curchar = $i;
                    }
                }
            } else {
                echo('Player has no server.'."\n");
            }
