...
if (empty($_SESSION['character'])) {
    echo "You need to choose a character in order to use the Virtual Porkhollow!<br />";
} else {
    ...
    if ($computability == 0) {
        if ($compugood == true) {
            echo "You need a computer in storage or equipped to wire boondollars to other players.</br>";
        }
        $compugood = false;
    }
	...
	Amount of boondollars to transfer: <input id="amount" name="amount" type="text" /><br /><input type="submit" value="Wire it!" /></form>';
        ...
        echo "Boondollars: " . strval($charrow['boondollars']);
    }
}
