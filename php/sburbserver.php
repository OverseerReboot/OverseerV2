...
if (empty($_SESSION['username'])) {
    ...
    if ($compugood) {
        ...
        if (empty($charrow['client'])) { //No client: Print out a form with potential options.
                    ...
                    if ($deploytag[0] == "DEPLOYABLE") {
                        ...
                        if ($currentstack < $fullstack || $drow['code'] == "11111111") { //the user doesn't have this in their storage yet
                            ...
                            if ($canafford) {
                                ...
                                if ($clientrow['inmedium'] == 1) {
                                    echo "Deploy failed: client lacks the required grist.<br />";
                                } else {
                                    echo "Deploy failed: you don't have access to that item yet!<br />";
                                }
                            }
                        } else {
                        ...
                        if (!empty($_POST['r-' . $args[0]])) { //This item is being recycled
                            ...
                            if ($irow['id'] == $args[0]) {
                                ...
                                if (intval($args[1]) >= $_POST['q-' . $args[0]]) {
                                    ...
                                    if ($nothing) {
                                        echo "nothing, because you originally deployed it for free or for a non-build cost.";
                                    } //items will ALWAYS have a grist cost listed, even if just a vanity cost of 0
                                    echo '<br />';
                                } else {
            ...
            if (strval($gates + 1) < 8) {
                $gategrist = $gaterow["gate" . strval($gates + 1)] - $clientrow['house_build'];
            } else {
                $gategrist = 0;
            }
            ...
            if ($clientrow['inmedium'] == 0) {
                echo '<option value="11">Pre-punched Card (Cost: --)</option>';
            }
