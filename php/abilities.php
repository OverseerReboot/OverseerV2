                ...
                if (!empty($_POST['target'])) {
                    ...
                    if ($targetrow = mysqli_fetch_array($targetresult)) { //Target was found
                        if ($targetrow['dreamingstatus'] != $charrow['dreamingstatus']) {
                            $reach = false;
                        } //Cannot reach the target. This MIGHT not matter.
                        if ($targetrow['dreamingstatus'] == "Awake") {
                            $rowID = $targetrow['wakeself'];
                        } //Find and fetch the strife row relevant to their dreaming status
                        if ($targetrow['dreamingstatus'] != "Awake") {
                        ...
                        if (!$notarget) {
                            if ($reach) {
                                ...
                                while (!empty($statuses[$i])) { //Evaluate this status
                                    ...
                                    if ($strpos($removestr, $tofind) === false) {
                                        $newstatus .= $statuses[$i] . "|";
                                    } //If NOT found as a debuff, add it to the new status string.
                                    $i++;
                                }
                                ...
                                while (!empty($bonuses[$i])) {
                                    ...
                                    if (intval($currentbonus[2]) > 0) {
                                        $newbonuses .= $bonuses[$i] . "|";
                    ...
                    case 8: //Seek Fortune's Path (ID 8)
                        ...
                        while ($allyrow = mysqli_fetch_array($allyresult)) {
                            if (true && $allyrow['id'] != $charrow['id']) { //NOTE - The TRUE condition will be "if the player currently has a computer"
                                ...
                                if ($allyrow['dreamself']  != 0) {
                                    $findstr .= strval($allyrow['dreamself']) . ", ";
                                }
                            }
                        }
