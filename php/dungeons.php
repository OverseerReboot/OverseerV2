...
function genEncounter($minpower, $maxpower, $search): string|false
{
    ...
    if ($row = mysqli_fetch_array($enemyresult)) { //see if we found anything
        $i = 1;
        while ($i < 10) {
            if (($row['basepower'] * $i) + ($i * $i) <= $maxpower) {
                $i++;
            } else {
                break;
            } //This value is over the maximum, stop here
        }
        $i--; //subtract off the tier that exceeded maxpower
        $maxtier = $i;
        $i = 9;
        while ($i > 1) {
            if (($row['basepower'] * $i) + ($i * $i) >= $minpower) {
                $i--;
            } else {
                break;
            } //This value is under the minimum, stop here
        }
        $mintier = $i;
        if ($maxtier < 1 || $mintier > 9 || $maxtier < $mintier) {
            return false;
        } //error protection~
        ...
        if ($charrow['dungeon'] == 0) {
            ...
            if ($charrow['gatescleared'] > 0) {
                ...
                while ($i <= $charrow['gatescleared']) {
                    if ($i < 7) {
                        echo '<option value="gate' . strval($i) . 'dungeon">Native dungeon: Gate ' . strval($i) . ' territory</option>';
                    }
            ...
            while (!empty($allrooms[$i])) {
                ...
                if ($currentroom == $map[1]) {
                    ...
                    if (!empty($troom)) {
                        ...
                        if (!$canmove) {
                            echo "You cannot transportalize there!<br />";
                        }
                    }
                }
                if ($newroom == $map[1]) {
            ...
            if (strpos('|' . $dungeonrow['enc'], '|' . $currentroom . ':') !== false) {
                ...
                if ($enc[1] == "EXISTS") { //encounter was already generated, so the remaining args are the enemy IDs
                    ...
                    if ($newID == "BOSS") {
                        $newID = $enc[3];
                    } //Second arg was the boss flag! Use the third arg instead.
                    ...
                    while (!$done) {
                        ...
                        if ($newenc != "") {
                            ...
                            break;
                        } //there was an error, break out of the loop
                        if ($tpower >= $maxi) {
                            $done = true;
                        } else {
                            $cchance = rand(1, 100);
                            if ($cchance < ($tpower * 50 / $maxi)) {
                                $done = true;
                            } //50% chance of bailing prematurely at $maxi
                        }
                    }
                    if (!$done) {
                ...
                if ($newID != 0) {
                    ...
                    while ($row = mysqli_fetch_array($enemiesresult)) {
                        if (!$enemypresent) {
                            $message .= "As you enter the room, you are assaulted by enemies:<br />";
                        }
                        $enemypresent = true;
                        if (!empty($row['grist']) && $row['grist'] != "None") {
                            $message .= $row['grist'] . " ";
                        }
                        $message .= $row['name'] . "<br />";
                    ...
                    if ($loot[1] != "EXISTS") { //loot has not yet been generated here, so arg 1 is the value of the loot
                        ...
                        while (!$done) {
                            ...
                            if ($tvalue >= $maxi) {
                                ...
                                $cchance = rand(1, 100);
                                if ($cchance < ($tvalue * 50 / $maxi)) {
                                    $done = true;
                                } //50% chance to abort early if we're at max
                            }
                        }
                        $loot = explode(":", $loots);
                    }
                ...
                while ($j <= $rightmost) {
                    ...
                    if (!empty($roomstr)) {
                        ...
                        if (strpos($roomstr, "VISITED") !== false) {
                            ...
                            if (strpos($map[3], "n") !== false && strpos($dungeonstring[$j][$i - 1], "VISITED") !== false) {
                                $blank = false;
                            } elseif (strpos($map[3], "s") !== false && strpos($dungeonstring[$j][$i + 1], "VISITED") !== false) {
                                $blank = false;
                            } elseif (strpos($map[3], "w") !== false && strpos($dungeonstring[$j - 1][$i], "VISITED") !== false) {
                                $blank = false;
                            } elseif (strpos($map[3], "e") !== false && strpos($dungeonstring[$j + 1][$i], "VISITED") !== false) {
                                $blank = false;
                            }
                        }
                        //$blank = false;
                    ...
                    if ($blank) {
                        ...
                        $tilename = "";
                        if (strpos($roomstr, "VISITED") !== false) {
                            if ((strpos('|' . $dungeonrow['enc'], '|' . $thisroom . ":BOSS") !== false) || (strpos('|' . $dungeonrow['enc'], '|' . $thisroom . ":EXISTS:BOSS") !== false)) {
                                $tilename .= "boss_";
                            } elseif (strpos('|' . $dungeonrow['enc'], '|' . $thisroom) !== false) {
                                $tilename .= "enemy_";
                            } elseif (strpos($roomstr, "ENTRANCE") !== false) {
                                $tilename .= "entrance_";
                            }
                            if (strpos('|' . $dungeonrow['loot'], '|' . $thisroom) !== false) {
                                $tilename .= "loot_";
                            }
                            if (strpos($map[3], "u") !== false || strpos($map[3], "d") !== false) {
                                $tilename .= "stairs_";
                            } elseif (strpos($roomstr, "TRANSPORT") !== false) {
                                $tilename .= "transport_";
                            }
                        } else {
                            $tilename .= "unknown";
            ...
            if (strpos($exits, "n") !== false) {
                ...
                echo "<img src='/images/dungeon/dgnbtn_north_blocked.png'>";
            }
            ...
            if (strpos($exits, "w") !== false) {
                ...
                echo "<img src='/images/dungeon/dgnbtn_west_blocked.png'>";
            }
            echo "</td><td>";
            ...
            if (strpos($exits, "e") !== false) {
                ...
                echo "<img src='/images/dungeon/dgnbtn_east_blocked.png'>";
            }
            ...
            if (strpos($exits, "s") !== false) {
                ...
                echo "<img src='/images/dungeon/dgnbtn_south_blocked.png'>";
            }
