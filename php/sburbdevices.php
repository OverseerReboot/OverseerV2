...
if (empty($_SESSION['username'])) {
    ...
    if (!empty($_POST['cruxopen']) && $crux) { //User is attempting to open the cruxtruder
        if ($charrow['sprite'] == 0) { //Cruxtruder is closed, we can tell because the player lacks a sprite
            ...
            echo "The Cruxtruder is already open!<br />";
        }
    }
    ...
    if (!empty($_POST['cruxtrude']) && $crux) { //User extruding cruxite
        ...
        if ($success) {
            ...
            echo "There isn't enough space in your house for that many dowels! You need to captchalogue or recycle something first.<br />";
        }
        ...
        if ($tcode == "00000000" && $arg[0] == 13) { //Double check that the item here is a dowel and that it is uncarved
            ...
            while ($i < 3) { //Max. 2 cards, so we check both card inputs
                ...
                if ($area == "i") { //Card is in inventory
                    ...
                    $code[$i] = "!!!!!!!!";
                } //using no card is the equivalent of having all holes punched
                $i++;
            }
            if ($aok) {
                ...
                echo "Looks like one or both of the cards you selected were moved since you last took inventory - please try the operation again.<br />";
            }
        } else {
            echo "Looks like your uncarved dowel was moved or carved since you last took inventory - please try the operation again.<br />";
        }
    }

    if (empty($_POST['alchnum'])) {
        $_POST['alchnum'] = 0;
    }
        ...
        if ($_SESSION['inv'][$i] == 13) { //this item is a cruxite dowel/totem
            ...
            if ($meta[0] % 2 == 1) { //this totem is available
                ...
                if ($itemrow = mysqli_fetch_array($itemresult)) {
                    ...
                    echo "The item corresponding to this code does not currently exist. You may create it using the <a href='quickitemcreate.php'>Quick Creation Form</a> or make a full submission.<br />";
                }
            } else {
                echo "Alchemy failed: that totem is not currently available in your inventory.<br />";
            }
        } else {
            echo "Alchemy failed: that item is not a cruxite totem!<br />";
        }
    }
        ...
        while (!empty($store[$i])) { //now sort through storage, since not all modi will allow the user more than 1 card at a time
            ...
            if ($arg[0] == 11) { //this is a captcha card
                ...
                if ($code == "00000000") { //dowel is uncarved
                    if ($cruxcount == 0) {
                        echo "<input type='hidden' name='firstdowel' value='$i' />";
                    } //mark the first dowel now so we don't have to run through storage when the form is submitted
                    $cruxcount += $arg[1];
                }
            }
            $i++;
