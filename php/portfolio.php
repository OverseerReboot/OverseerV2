...
if (empty($_SESSION['username'])) {
    ...
    $headp = false;
    $facep = false;
    if (empty($charrow['equips'])) {
        $charrow['equips'] = "main:0|off:0|";
    }
    ...
    if (!empty($_POST['newabs'])) { //player is assigning an abstratus manually
        if (substr_count($charrow['abstratus'], "|") < $charrow['abslots']) {
            ...
            echo "Error: you have no open slots to assign a kind abstrati.<br />";
        }
    }
        ...
        if ($meta[0] % 2 == 1) { //odd-numbered first arg, item is available
            ...
            if (!empty($erow[$_SESSION['inv'][$i]]['abstratus']) && $erow[$_SESSION['inv'][$i]]['abstratus'] != "notaweapon") { //item is a weapon
                ...
                while (!empty($itemabses[$j])) {
                    if ((strpos($charrow['abstratus'], "|" . $itemabses[$j]) !== false) or (strpos($charrow['abstratus'], "|jokerkind") !== false)) {
                        $canequip = true;
                    }
                    $j++;
                }
                ...
                if ($j == 1 && $canequip == false) { //item has only one abstratus, and the user doesn't have it yet
                    ...
                    if ($abstaken < $charrow['abslots']) {
                        ...
                        echo "You cannot add that weapon to your strife deck, and you have no free strife cards in your portfolio to assign its abstratus.<br />";
                    }
                } else {
                    if ($canequip == false) {
                        echo "Your strife deck rejects the multi-abstratus weapon! You must assign one of its abstrati to your strife portfolio manually first.<br />";
                    }
                }
            } else {
                echo "Error: that item is not a weapon.<br />";
            }
        } else {
            echo "Error: that item is not available in your sylladex.<br />";
        }
        if ($canequip) {
    ...
    if (!empty($_POST['deckaction'])) { //player wants to do something with their strife deck
        if ($_POST['deckaction'] == "main") { //equipping a weapon to main slot
            ...
            if (itemSize($erow[$newequip]['size']) == itemSize("large")) { //weapon is two-handed
                ...
                if (count($deck) < 3) {
                    str_replace("off:1|", "off:0|", $wequips);
                } //if you're changing weapons from off to main with an empty inventory
            ...
            if (itemSize($erow[$newequip]['size']) == itemSize("large")) { //off-handed weapon is two-handed
                ...
                if (strpos($wequips, "off:2|") !== false) { //main weapon is two-handed
                    ...
                    if ($_POST['deckitem'] != 0) {
                        ...
                        echo "Please drop or unequip your main weapon before equipping it in your off-hand.<br>";
                    }
                }
            }
            ...
            if ($_POST['deckitem'] != 0 || stripos($wequips, 'off:1') == false) { //if you're dropping the main weapon with an off weapon equipped, stop it
                ...
                if ($success) {
                    ...
                    //echo "wequips currently " . $wequips . '<br>';
                    if ($_POST['deckitem'] == 0) {
                        $wequips = str_replace("main:1|", "main:0|", $wequips);
                    } //this item was equipped in the main slot, unequip it
                    if ($_POST['deckitem'] == 1) {
                        $wequips = str_replace("off:1|", "off:0|", $wequips);
                    } //was equipped in off slot, unequip it
                    if ($_POST['deckitem'] == 0) {
                        $wequips = str_replace("off:2|", "off:0|", $wequips);
                    } //the only way to equip 2-handed is using the main slot, so just in case we'll do this, no problem
                    //echo "wequips changed to " . $wequips . '<br>';
                } else { //just in case
                ...
                echo "Please unequip or drop your offhand weapon first.<br>";
            }
        }
        /*
    ...
    if (!empty($_POST['eqwr'])) { //player is equipping a wearable
        ...
        if ($meta[0] % 2 == 1) { //odd-numbered first arg, item is available
            ...
            if (!empty($erow[$_SESSION['inv'][$i]]['wearable']) && $erow[$_SESSION['inv'][$i]]['abstratus'] != "none") { //item is a weapon
                ...
                $eequips = "";
                while (!empty($itemabses[$j])) {
                    if (strpos($wequips, $itemabses[$j]) !== false) {
                        $canequip = false;
                    } //already has a wearable in that slot, denied
                    else {
                        $eequips .= $itemabses[$j] . ":" . $_SESSION['inv'][$i] . "|";
                    } //item will take up this slot
                    $j++;
                }
                if ($canequip) {
                ...
                echo "Error: that item is not a wearable.<br />";
            }
        } else {
            echo "Error: that item is not available in your sylladex.<br />";
        }
    ...
    while (!empty($equip[$i])) {
        ...
        if ($thisequip[0] == "main") {
            if ($_POST['unequip'] == "main") {
                ...
                if (stripos($wequips, 'off:1') == false) {
                    ...
                    echo "Please unequip your offhand weapon before unequipping your main one.<br>";
                }
            }
            if ($thisequip[1] == 1) {
                $thisequip[1] = $deck[0];
            } //use the ID from the strife deck
            ...
            if ($thisequip[1] == 2) { //the main weapon is 2-handed
                ...
                if ($thisequip[1] == 1) {
                    if (($equip[0] == "main:0") and (count($deck) < 3)) {
                        $thisequip[1] = $deck[0];
                    } else {
        ...
        if ($thisequip[1] != 0) { //there's something here that wasn't just unequipped
            ...
            if (stripos($thisequip[0], 'head') !== false) {
                $headp = true;
            }
            if (stripos($thisequip[0], 'main') !== false) {
                $mainp = true;
            }
            if (stripos($thisequip[0], 'face') !== false) {
                $facep = true;
            }
            if (stripos($thisequip[0], 'body') !== false) {
                $bodyp = true;
            }
            if ($accp && $bodyp && ($headp or $facep) && $mainp) {
                setAchievement($charrow, 'fullport');
            }
    ...
    if (empty($deck[0]) && !empty($deck[1])) {
        $i = 1;
    } //If the user equips their only weapon to their offhand, the first entry will be blank and the
    ...
    while ($i < $w) {
        if ($i == 0) {
            echo "<form action='portfolio.php' method='post' id='addform'><input type='hidden' name='eqwpn' value='ys' />Add weapon to strife deck: <select name='eqweapon' id='eqweapon'>";
        }
    ...
    while ($i < $e) {
        if ($i == 0) {
            echo "<form action='portfolio.php' method='post' id='equipform'><input type='hidden' name='eqwr' value='ys' />Equip a wearable: <select name='eqwear' id='eqwear'>";
        }
        echo "<option value='" . $equipchoice[$i] . "'>" . $erow[$_SESSION['inv'][$equipchoice[$i]]]['name'] . "</option>";
        $i++;
    }
    if ($i > 0) {
        echo "</select><input type='submit' value='Equip' /></form>";
    }
}
