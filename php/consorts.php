...
if (empty($_SESSION['character'])) {
    ...
    if (empty($_GET['land']) && $_GET['land'] !== "0") {
        $_GET['land'] = "";
    }
    if (empty($_GET['search'])) {
        $_GET['search'] = "";
    }
    if (empty($_GET['completequest'])) {
        $_GET['completequest'] = "";
    }
    ...
    if (!is_numeric($_GET['land'])) {
        ...
        if ($_GET['land'] !== "" || $_GET['land'] != 0) {
            $land = mysqli_real_escape_string($connection, $_GET['land']);
        } else {
            $land = 0;
        }
        if ($land >= 0) {
        ...
        } elseif (strpos($charrow['canaccess'], $land) !== false) {
            $canaccess = true;
        } else {
            $canaccess = false;
        }
        if ($land > 0) {
            ...
            $location = "the Land of " . $charland['land1'] . " and " . $charland['land2'];
            $explorelevel = explorationLevel($charland['landexplored']);
        } elseif ($land < 0 && $land >= -3 && $strpos($charrow['canaccess'], strval($land)) !== false) {
            $playerland = false;
            if ($land == -1) {
                $location = "Prospit";
            } elseif ($land == -2) {
        ...
        if ($canaccess) {
            ...
            $gate = 7;
            while ($gate > 0) {
                if ($gaterow[$gate - 1] <= $charland['house_build']) {
                    break;
                }
                $gate--;
            }
            if (!empty($charland['consort'])) {
                $consort = $charland['consort'];
            } else {
                $consort = "consort";
            }
        }
        ...
        if ($charland['session'] != $charrow['session']) {
                ...
                if (!empty($_GET['completequest']) && $charrow['currentquest'] != 0) { // quest exists and is being completed
                    if ($_GET['completequest'] == "debug") {
                        ...
                        if (in_array($_GET['completequest'], $_SESSION['inv'])) {
                            ...
                            if ($completed) {
                                ...
                                if ($gristcost <= $gaterow[$gate - 1]) {
                                    $booncost = totalBooncost($itemrow['gristcosts'], $charland);
                                } else {
                                    $completed = false;
                                }
                            } else {
                    ...
                    if ($completed) {
                        if ($context == "questitem") {
                            echo "The consort is overjoyed, this item is perfect!<br />";
                        } else {
                            $booncost = 0;
                        }
                        ...
                        } elseif ($context = "questitem" && $_GET['completequest'] != "debug") {
                            $slot = array_search($_GET['completequest'], $_SESSION['inv']);
                            removeItem($slot);
                            echo "Debug: item removed. <br />";
                        }
                    } else {
                        echo '<a href="?search=quest&land=' . $land . '">Try again</a> or ';
                    }
            ...
            } elseif ($charrow['availablequests'] > 0) {
        ...
        } elseif ($_GET['search'] == 'shop' && $charrow['dreamingstatus'] == "Asleep") {
            ...
            echo '<a href="?land=' . $land . '">(Go back)</a><br />';
        } elseif ($_GET['search'] == 'shop' && $land >= 0) {
            echo "Shop beta:<br />";
            if (!empty($_POST['shopaction'])) {
                $purchased = false;
                $forcerefresh = false;
                if (strpos("|" . $charland['shopstock'], "|" . $_POST['shopaction'] . ":") !== false) {
                    ...
                    if ($charrow['boondollars'] < $shopprice) {
                        echo "Sorry " . $charrow['name'] . ", I can't give credit! Come back when you're a little... mmm... RICHER!</br>";
                    } else {
                        $purchased = true;
                    }
                } elseif ($_POST['shopaction'] == "refresh") {
                    ...
                    echo "This shop is not selling that item!<br />";
                }
                if ($purchased) {
                    $newitem = addItem($charrow, $_POST['shopaction'], "", false);
                    if ($newitem) {
                        sumStat($charrow, 'consortboon', $shopprice);
                        if (getStat($charrow, 'consortboon') > 99999999) {
                            setAchievement($charrow, 'boonshop');
                        }
                    ...
                    echo "The purchase failed!<br />";
                }
            }
            if ($gate < 1) {
                $gate = 1;
            }  // even on a land with no gates available, shops will stock as if there is at least one, in case the land is reached by a flying player for instance
            ...
            if (!empty($charland['shopstock'])) {
                ...
                while (strlen($hourstr) < 2) {
                    $hourstr = "0" . $hourstr;
                }
                $minutestr = strval(floor($time / 60) % 60);
                while (strlen($minutestr) < 2) {
                ...
                while ($thisitem = mysqli_fetch_assoc($shopitems)) {
                    ...
                    if ($canwield) {
                        ...
                        if ($powerdiff < -9999) {
                            echo "It looks like an utter piece of shit, though.";
                        } elseif ($powerdiff < -5000) {
                            echo "However, you outgrew the need for such flimsy weaponry long ago.";
                        } elseif ($powerdiff < -1000) {
                            echo "You doubt you could see any use out of it, though.";
                        } elseif ($powerdiff < -100) {
                            echo "It looks a bit weak for your needs.";
                        } elseif ($powerdiff < 0) {
                            echo "You think your current equipment might be better, but you never know...";
                        } elseif ($powerdiff == 0) {
                            echo "It looks exactly as strong as your current equipment.";
                        } elseif ($powerdiff > 9999) {
                            echo "Whoa, where has this been all your life?!";
                        } elseif ($powerdiff > 9000) {
                            echo "You'd be lucky to ever get your hands on one of these!";
                        } elseif ($powerdiff > 5000) {
                            echo "It looks ridiculously stronger than your current weapon!";
                        } elseif ($powerdiff > 1000) {
                            echo "You could definitely use something as strong as this!";
                        } elseif ($powerdiff > 100) {
                            echo "It looks pretty strong. It'd probably be a decent upgrade.";
                        } elseif ($powerdiff > 0) {
                            echo "You think it might be a little stronger than what you're currently using, but you can't say for sure.";
                        }
                        echo "</br>";
        ...
        } elseif ($_GET['search'] == 'exchange' && $exchangeloc == $land) {
            echo "Exchange not ready";
            echo '<a href="?land=' . $land . '">(Go back)</a><br />';
        } elseif ($_GET['search'] == 'exchange') {
            if ($land < 0) {
                echo "The grist exchange can only be found on player lands!<br />";
            } else {
                echo "There's no grist exchange on this land!<br />";
                if ($exchangeloc == 0 && $charland['economy'] >= 4000000) {
                    echo "In fact, there's no grist exchange in the session yet. Would you like to establish one?<br />";
                }
