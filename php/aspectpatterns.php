...
if (empty($charrow)) {
    ...
    if (empty($_SESSION['aspectrow']['name'])) {
        $_SESSION['aspectrow']['name'] = "";
    }
    if (empty($_SESSION['classrow']['name'])) {
    ...
    if (!empty($_POST['deletepattern'])) { //Deleting a pattern. We do not use it in this case!
        ...
        while (!empty($valuestrs[$i])) {
            ...
            if ($values[$valuestrs[$i]] < 0) {
                $fail = "That pattern has a negative value! Nice try though.<br />";
            }
        ...
        if ($cost < 10) {
            $cost = 10;
        } //No ability may cost less than 20% of the aspect vial.
        $cost = floor(($cost / 100) * $striferow['maxenergy']); //Converts cost from percentage to actual value
        if ($cost > $striferow['energy'] && empty($fail)) {
            $fail = "You do not have enough Aspect Vial remaining to use that pattern!";
        }
        ...
        if (empty($fail)) { //NOTE - $fail is an error message. If it isn't empty, we print it since it contains the error. If it is empty, we didn't fail!
            ...
            if ($active) {
                $aspectpower *= ($classrow['activefactor'] / 100);
            }
            if ($passive) {
            ...
            if ($targetrow['side'] == $striferow['side'] || $striferow['strifeID'] == 0) { //Ally targeted: Apply all effects to them. Including negative ones.
                ...
                if ($damage > 0) {
                    $notification = $notification . " Ouch!";
                }
                if ($heal > 0) {
                    setAchievement($charrow, 'aspectheal');
                    $notification = $notification . " Health up!";
                }
                if ($offenseup != 0) {
                    $targetrow['bonuses'] = $targetrow['bonuses'] . "OFFENSE:0:$offenseup|";
                    $notification = $notification . " Offense up!";
                }
                if ($defenseup != 0) {
                    $targetrow['bonuses'] = $targetrow['bonuses'] . "DEFENSE:0:$defenseup|";
                    $notification = $notification . " Defense up!";
                }
                if ($powerup != 0) {
                    $targetrow['bonuses'] = $targetrow['bonuses'] . "POWER:0:$powerup|";
                    $notification = $notification . " Power up!";
                }
                ...
                if ($offenseup != 0) {
                    $striferow['bonuses'] = $striferow['bonuses'] . "OFFENSE:0:$offenseup|";
                }
                if ($defenseup != 0) {
                    $striferow['bonuses'] = $striferow['bonuses'] . "DEFENSE:0:$defenseup|";
                }
                if ($powerup != 0) {
            ...
            if ($targethealth > $targetrow['maxhealth']) {
                $targethealth = $targetrow['maxhealth'];
            }
            if ($targethealth < 1) {
                $targethealth = 1;
            }
    ...
    if ($charrow['aspectpatterns'] != "") {
        echo "Or select a pattern you have previously saved:</br>";
    }
    $patternarray = explode("|", $charrow['aspectpatterns']);
    $i = 0;
    while (!empty($patternarray[$i])) {
