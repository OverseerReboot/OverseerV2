...
if (empty($_SESSION['character'])) {
    ...
    if (isset($_GET['old'])) {
        ...
        if (isset($_SESSION['oldinvui'])) {
            $newui = $_SESSION['oldinvui'];
        } else {
            $newui = true;
        }
    }
        ...
        if ((int)$meta[0] % 2 == 1) { //item is available
            ...
            if ($_POST['invaction'] == "use") { //player wants to consume this item
                if (!empty($irow[$i]['consumable'])) { //item can be consumed
                    ...
                    } //only the waking self can use items, for now
                ...
                if (!empty($_POST['target'])) {
                    ...
                    if (!empty($meta[1])) {
                        $extras = $meta[1];
                    } //metadata now contains all of the extra effects of an item just like storage
                    ...
                    if ($success) {
                        ...
                        echo "You don't have enough room in your house to drop that!<br />";
                    }
                }
                ...
                if ($read[0] == "READ") {
                    ...
                    echo "Your " . $irow[$i]['name'] . " is not good reading material.<br />(But if you think it should be, feel free to submit a text for it to <a href='http://babbyoverseer.tumblr.com'>the Overseer Item Blog</a>!)<br />";
                }
                ...
                if (!empty($charrow['inventory'])) {
                    ...
                    if (in_array($i, $boom)) {
                        ...
                        if ($irow['name'] == 'Perfectly Unique Object') {
                            setAchievement($charrow, 'allgrist');
                        }
                        $g = 0;
            ...
            echo "That item is not available.<br />";
        }
    }
