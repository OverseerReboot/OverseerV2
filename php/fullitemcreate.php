...
if ($_SESSION['username'] != "") {
    ...
    if (!empty($_POST['name'])) {
        ...
        if (abs(intval($power)) > 9999) {
            $invreason .= "Base power level cannot be higher than 9999 or lower than -9999<br />";
        }
        ...
        while (!empty($bonusarray[$b])) {
            if (!empty($_POST[$bonusarray[$b]])) {
                ...
                if (abs(intval($_POST[$bonusarray[$b]])) > 9999) {
                    $invreason .= $bonusarray[$b] . " cannot be higher than 9999 or lower than -9999<br />";
                }
            }
            $b++;
        }
        ...
        if (!empty($_POST['abstratus'])) {
            $abs = mysqli_real_escape_string($con, $_POST['abstratus']);
        } else {
            $abs = "notaweapon";
        }
        if (!empty($_POST['wearable'])) {
            $wear = mysqli_real_escape_string($con, $_POST['wearable']);
        } else {
            $wear = "none";
        }
        $size = $_POST['size'];
        ...
        if (!empty($_POST['lootonly'])) {
            $loot = "1";
        } else {
            $loot = "0";
        }
        if (!empty($_POST['refrance'])) {
            $ref = "1";
        } else {
            $ref = "0";
        }
  ...
  <strike>- no bladekinds plz</strike><br /><br />";
    ...
    if (empty($itemrow['size'])) {
        $itemrow['size'] = "average";
    }
